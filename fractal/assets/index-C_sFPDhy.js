...
`;function an(o,e,n){if(Array.isArray(n))if(n.length===2)o[e]={value:new se(n[0],n[1])};else if(n.length===3)o[e]={value:new dt(n[0],n[1],n[2])};else if(n.length>=4&&n.length%2===0){const s=[];for(let t=0;t<n.length;t+=2)s.push(new se(n[t],n[t+1]));o[e]={value:s}}else o[e]={value:n};else o[e]={value:n}}function sn(o,e,n){if(o.uniforms[e])if(Array.isArray(n))if(n.length===2)o.uniforms[e].value.fromArray(n);else if(n.length===3)o.uniforms[e].value.fromArray(n);else if(n.length>=4&&n.length%2===0){const s=[];for(let t=0;t<n.length;t+=2)s.push(new se(n[t],n[t+1]));o.uniforms[e].value=s}else o.uniforms[e].value=n;else o.uniforms[e].value=n}const ln=({fractal:o,center:e,viewWidth:n,viewHeight:s,parameters:t,controlPoints:a,colorScheme:u})=>{const l=Ee(p=>p.invalidate),i=c.useMemo(()=>{const p={FRACTAL_SHADER:o.getShaderCode(),EVALUATE_COLOR:tn(u)},h={center:{value:new se(...e)},viewWidth:{value:n},viewHeight:{value:s}};o.getGuiParameters().forEach(x=>{h[x.name]={value:t[x.name]}});const b=o.getControlPointUniforms(a);if(b)for(const[x,v]of Object.entries(b))an(h,x,v);return new lt({uniforms:h,vertexShader:rn,fragmentShader:on.replace(/%(\w+)%/g,(x,v)=>String(p[v]))})},[o,u]);c.useEffect(()=>{i.uniforms.center.value.fromArray(e),i.uniforms.viewWidth.value=n,i.uniforms.viewHeight.value=s,o.getGuiParameters().forEach(h=>{i.uniforms[h.name].value=t[h.name]});const p=o.getControlPointUniforms(a);if(p)for(const[h,b]of Object.entries(p))sn(i,h,b);l()},[i,e,n,s,t,a,l,o]);const d=c.useMemo(()=>{const p=new ct;return p.setDrawRange(0,3),p},[]);return r.jsx("mesh",{material:i,geometry:d})},Ne=De,cn=({initialFractal:o,initialColorScheme:e,onFractalChange:n,onColorSchemeChange:s})=>{const{theme:t}=Q(),[a,u]=c.useState(o),[l,i]=c.useState(a.getInitialGuiParameters()),[d,p]=c.useState(a.getInitialControlPoints()),[h,b]=c.useState(e||pe),x=Ne.map(w=>({name:w.title,value:w.name})),v=w=>{const R=Ne.find(L=>L.name===w);R&&(u(R),n(R),i(R.getInitialGuiParameters()),p(R.getInitialControlPoints()),g(L=>L+1))},S=w=>{b(w),s(w)},j=Object.entries(nn).map(([w,R])=>({name:R,value:w})),[y,g]=c.useState(0),f=a.getGuiParameters().map(w=>Pt(w)?r.jsx("div",{className:"min-w-[150px] flex-[2_1_0] my-1",children:r.jsx(gt,{label:w.displayName,min:w.min,max:w.max,value:l[w.name],onChange:R=>i(L=>({...L,[w.name]:R})),theme:t,posToVal:w.posToVal,valToPos:w.valToPos})},w.name):null),T=a.getControlPointValues(d),[U,B]=a.getControlPointMinMax();return r.jsxs("div",{className:F("w-full h-full flex flex-col landscape:flex-row overflow-hidden",t==="dark"?"bg-gray-900 text-white":"bg-gray-100 text-black"),children:[r.jsxs("div",{className:"flex-shrink-0 p-2 landscape:p-4 landscape:w-56 lg:w-64 overflow-y-auto overflow-x-hidden",children:[r.jsx("h2",{className:"hidden landscape:block text-lg font-bold w-full mb-2",children:"Controls"}),r.jsxs("div",{className:"flex flex-wrap items-center gap-2",children:[r.jsx("div",{className:"min-w-[140px] flex-1",children:r.jsx(Me,{options:x,selected:a.title,onChange:w=>v(w),theme:t})}),r.jsx("div",{className:"min-w-[140px] flex-1",children:r.jsx(Me,{label:"Colors",options:j,selected:j.find(w=>w.value===h)?.name,onChange:w=>S(w),theme:t})}),f,T&&T.length>0&&r.jsx("div",{className:"w-full gap-0 py-2 rounded-md hidden landscape:flex landscape:flex-col",style:{backgroundColor:t==="dark"?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.05)"},title:"Drag the control point on the canvas to change. Double-click to reset.",children:T.map(w=>r.jsxs("div",{className:"w-full flex flex-wrap justify-between items-baseline gap-x-1",children:[r.jsx("p",{className:F("text-sm font-medium",t==="dark"?"text-gray-300":"text-gray-700"),children:w.label}),r.jsx("p",{className:F("text-base font-mono",t==="dark"?"text-gray-200":"text-gray-800"),children:w.value})]},w.label))})]})]}),r.jsx("div",{className:"flex-grow flex items-center justify-center pt-2.5 landscape:p-2 relative",style:{minWidth:0,minHeight:0},children:r.jsx(Mt,{baseViewSize:4,frameloop:"demand",onInfoOverlay:(w,R)=>{const L=["[",", ","]"],z=ke(R.center[0]-R.viewWidth/2,R.center[0]+R.viewWidth/2,7,L),m=ke(R.center[1]-R.viewHeight/2,R.center[1]+R.viewHeight/2,7,L),_={"top-left":le(...w),"top-right":`Re: ${z}
Im: ${m}`},M=a.getCanvasOverlayString(d,l);return M&&(_["bottom-center"]=M),_},controlPoints:d,initialControlPoints:a.getInitialControlPoints(),onControlPointsChange:p,controlPointGroups:a.getControlPointGroups(d),minControlPoints:U,maxControlPoints:B,gl:{powerPreference:"default",antialias:!1},children:({center:w,viewWidth:R,viewHeight:L})=>r.jsx(ln,{fractal:a,center:w,viewWidth:R,viewHeight:L,parameters:l,controlPoints:d,colorScheme:h})},y)})]})},dn=`This application allows you to explore the intricate beauty of various fractals.
...
`;var un={client:"ca-pub-5400548434612970",slot:"5745481735"};const ae=De;function hn(){const{theme:o}=Q(),e=c.useCallback(()=>{const y=["newton","mandelbrot","julia"],g=ae.filter(f=>y.includes(f.name));return g.length>0?g[Math.floor(Math.random()*g.length)]:ae[0]},[]),n=c.useRef(null);n.current===null&&(n.current=e());const s=c.useCallback(()=>{const g=ne().get("type");return ae.find(f=>f.name===g)||n.current},[n]),[t,a]=c.useState(s),[u,l]=c.useState(()=>ne().get("colors")||pe),[i,d]=c.useState(null);c.useEffect(()=>{const y=()=>{const g=ne(),f=g.get("type"),T=g.get("colors"),U=g.get("help"),B=ae.find(w=>w.name===f)||n.current;a(B),l(T||pe),d(U==="math"?"math":U==="controls"?"controls":null)};return window.addEventListener("hashchange",y),y(),()=>window.removeEventListener("hashchange",y)},[]);const p=y=>{de({type:y.name})},h=y=>{de({colors:y})},b=()=>{de({help:null})},x=y=>{const g=ne();return g.set("help",y),y==="math"&&!g.has("type")&&g.set("type",t.name),`#${g.toString()}`},v=[{key:"controls",name:"Exploring the Fractal",href:x("controls"),icon:r.jsx(rt,{})},{key:"math",name:`About ${t.title} Fractal`,href:x("math"),icon:r.jsx(ot,{})}],S=()=>{if(i==="math"){const y=`## ${t.title} Fractal

${t.math}`;return r.jsx(be,{remarkPlugins:[ze,$e],rehypePlugins:[we],children:y})}if(i==="controls"){const y=bt({canvasName:"fractal view",parameterName:"complex parameter"}),g=`${dn}

${y}`;return r.jsx(be,{remarkPlugins:[ze,$e],rehypePlugins:[we],children:g})}return null},j=()=>i==="math"?`About ${t.title} Fractal`:i==="controls"?"Exploring the Fractal":"";return r.jsxs(r.Fragment,{children:[r.jsx(mt,{appName:"Fantastic Fractals",help:v,adSenseConfig:un,children:r.jsx(c.Suspense,{fallback:r.jsx("div",{children:"Loading..."}),children:r.jsx(cn,{initialFractal:t,initialColorScheme:u,onFractalChange:p,onColorSchemeChange:h},t.name)})}),r.jsx(pt,{isOpen:i!==null,onClose:b,title:j(),size:"xl",children:r.jsx("div",{className:F("prose dark:prose-invert max-w-none",o==="dark"?"dark":""),children:S()})})]})}function fn(){return r.jsx(ht,{children:r.jsx(hn,{})})}at.createRoot(document.getElementById("root")).render(r.jsx(st.StrictMode,{children:r.jsx(fn,{})}));
